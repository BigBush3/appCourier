...
export default class MyOrderScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    BackHandler.addEventListener('hardwareBackPress', () => { this.props.navigation.navigate('FreeOrder'); return true; });
  ...
  _checkFree(arr){
    let ff = false;
    arr.map((item, index) => {
      if (item.STATUSDELIVERYMANID == '-6' && item.DELIVERYMANID == this.state.user.USERSID) {
          ff = true;
      }
    })
    return !ff;
  ...
  render() {
    ...
    var orders = this.state.list.map((item, index) => {
      if (item.STATUSDELIVERYMANID == '-6' && item.DELIVERYMANID == this.state.user.USERSID) {
        console.log("TTR")
        return (
          <UiProductCard key={index} number={item.NUMBER} title={item.DESTENATION} date={formatDateSQL(item.TOTIME)[0]} time={formatDateSQL(item.TOTIME)[1]} adress={item.RECEIVERADDRESS} onPress={() => navigate('Order', { backPlace: "MyOrder", pageType: 1, order: item })} />
        )
      }
    })
        ...
        <SafeAreaView style={styles.safeArea} forceInset={{ top: 'never' }}>
          <View style={styles.content}>
            <View style={styles.ordersList}>
              <ScrollView style={styles.scrollList} contentContainerStyle={{ paddingTop: 8, paddingBottom: 16, }}>
               
                {this._checkFree(this.state.list) ?
                  <View style={styles.result}>
                    <View style={styles.searchResult}>
                      <View style={styles.noResult}>
                        <Text style={styles.noResultText}>Нет моих заказов</Text>
                      </View>
                    </View>
                  </View>
                  : orders
